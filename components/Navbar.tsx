...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  const handleSyncMasters = async () => {
    ...
    try {
        const result = await syncMastersFromAllCompanies();
        if (result && result.companies && result.companies.length > 0) {
            setSyncSuccess(true);
            setTimeout(() => setSyncSuccess(false), 3000);
            console.log(`✓ Sync successful: ${result.companies.length} companies cached`);
        } else {
            console.warn("⚠ Sync returned no companies");
        }
    } catch (e) {
