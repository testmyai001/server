...
export const checkTallyConnection = async (): Promise<{ online: boolean; msg: string; activeCompany?: string }> => {
  try {
     ...
     // Use a simple Tally XML request to test connection (avoids /health which doesn't exist)
     ...
     if (response.ok) {
       ...
       if (text.includes('<RESPONSE>') || text.includes('<TALLYMESSAGE>')) {
         return { online: true, msg: "Tally Prime Connected ✓" };
       }
     }
     
     return { online: false, msg: "Tally response invalid" };
  } catch (error: any) {
     if (error.name === 'AbortError') {
       return { online: false, msg: "Connection timeout - Tally not responding" };
     }
     return { online: false, msg: "Cannot connect to Tally Prime. Ensure it's running on port 9000." };
  }
};
...
export const syncMastersFromAllCompanies = async (): Promise<TallyMasterSync> => {
    ...
    const syncData: TallyMasterSync = { companies, ledgers: ledgersMap, lastSync: Date.now() };
    // Save to localStorage for offline access
    localStorage.setItem('autotally_masters_sync', JSON.stringify(syncData));
    console.log(`✓ Masters synced: ${companies.length} companies, ${Object.keys(ledgersMap).length} companies with ledgers`);
    return syncData;
};
